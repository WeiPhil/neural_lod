...
struct BasicSceneState {
    ...
    char output_directory[MAX_FILENAME_SIZE] = "../../neural_lod/results/";
    char output_image_filename[MAX_FILENAME_SIZE] = "output";

    bool state(RenderBackend *renderer, std::vector<RenderExtension *> &extensions)
    {
#ifndef ENABLE_NEURAL_LOD  // Currently causes imgui.ini reading issues with neural lod uis
        ...
        if (IMGUI_STATE_BEGIN_HEADER(ImGui::CollapsingHeader,
            ...
            IMGUI_STATE_(
                ImGui::InputText, "Output directory", output_directory, MAX_FILENAME_SIZE);
        ...
        if (IMGUI_STATE_BEGIN_HEADER(ImGui::CollapsingHeader,
            ...
            const int last_active =
                std::max(std::min(op, array_ilen(tonemapping_operators) - 1), 0);
            if (IMGUI_STATE_BEGIN_ATOMIC_COMBO(ImGui::BeginCombo,
                                               "operator",
                                               tonemapping_operators,
                                               tonemapping_operators[last_active])) {
                for (int i = 0; i < array_ilen(tonemapping_operators); ++i) {
                    if (IMGUI_STATE(
                            ImGui::Selectable, tonemapping_operators[i], i == last_active)) {
                        op = i;
            ...
            renderer_changed |= IMGUI_STATE(
                ImGui::SliderFloat, "exposure", &renderer->params.exposure, -15.0f, 15.0f);
            IMGUI_STATE_END_HEADER(&renderer->params);
        }
        ...
        // #ifdef ENABLE_NEURAL
        //         if (IMGUI_STATE_BEGIN_HEADER(ImGui::CollapsingHeader, "NeRF",
        //         &renderer->options.enable_neural, ImGuiTreeNodeFlags_DefaultOpen)) {
        //             bool enable_RT_space_skipping =
        //             !renderer->options.NeRF_no_empty_space_skipping; if
        //             (IMGUI_STATE1(ImGui::Checkbox, "RT space skipping",
        //             &enable_RT_space_skipping)) {
        //                 renderer->options.NeRF_no_empty_space_skipping =
        //                 !enable_RT_space_skipping; renderer_changed = true;
        //             }
        //             renderer_changed |= IMGUI_STATE1(ImGui::Checkbox, "recurse on neural",
        //             &renderer->options.enable_recursion_on_neural);
    ...
    static std::string make_scene_id(const std::string &scene_name)
    {
        ...
        for (char const *c = scene_id.c_str() + scene_id.size(); c-- != scene_id.c_str();) {
            if (*c == '/') {
                if (delims == 1) {
                    scene_id.erase(scene_id.begin(),
                                   scene_id.begin() + (c - scene_id.c_str() + 1));
                    break;
                }
                ++delims;
            }
...
void imstate_scene_loader_parameters(SceneLoaderParams &params,
