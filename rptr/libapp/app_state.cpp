...
bool BasicApplicationState::state(RenderBackend *renderer)
{
    ...
    if (IMGUI_STATE(ImGui::DragInt, "target spp", &user_target_spp, 1.0f, -1, INT_MAX / 2)) {
        if (user_target_spp <= 0)
            renderer->params.spp_accumulation_window =
                std::max(renderer->params.spp_accumulation_window, 64);
        else
...
#endif
    other_changes |= IMGUI_STATE1(ImGui::Checkbox, "pause rendering", &pause_rendering);
    IMGUI_VOLATILE(ImGui::SameLine());
    other_changes |= IMGUI_STATE1(ImGui::Checkbox, "continuous restart", &continuous_restart);
    other_changes |= IMGUI_STATE1(
        ImGui::Checkbox, "render wavefront extensions", &render_wavefront_extensions);
    other_changes |=
        IMGUI_STATE1(ImGui::Checkbox, "render cuda extensions", &render_cuda_extensions);
    ...
    renderer_changed |= IMGUI_STATE1(ImGui::Checkbox, "glossy-only mode", &glossy_mode);
    renderer->params.glossy_only_mode = static_cast<int>(glossy_mode);

    renderer_changed |= IMGUI_STATE1(ImGui::Checkbox, "unroll bounces", &renderer->options.unroll_bounces);
#if defined(ENABLE_DYNAMIC_MESHES)  // || defined(ENABLE_DYNAMIC_INSTANCS)
    renderer_changed |= IMGUI_STATE1(ImGui::Checkbox, "force bvh rebuild", &renderer->options.force_bvh_rebuild);
    ...
    if (!renderer_variants.empty()) {
        ...
        int last_active =
            std::max(std::min(active_backend_variant, (int)renderer_variants.size() - 1), 0);
#if 1
        last_active =
            std::max(std::min(active_backend_variant, (int)renderer_variants.size() - 1), 1);
#endif
        char const *supported_flags = renderer_variants_support.data();
        bool find_new_supported = !supported_flags[last_active];
        auto pretty_variant_name =
            ImState::InDefaultMode() && last_active < (int)renderer_variants_pretty.size()
                ? renderer_variants_pretty[last_active].c_str()
                : renderer_variants[last_active].c_str();
        if (IMGUI_STATE_BEGIN_ATOMIC_COMBO(
                ImGui::BeginCombo, "variant", renderer_variants.data(), pretty_variant_name)) {
#if 1
            for (int i = 1; i < (int)renderer_variants.size(); ++i) {
#else
            for (int i = 0; i < (int)renderer_variants.size(); ++i) {
#endif
                ImGuiSelectableFlags item_flags =
                    !supported_flags[i] ? ImGuiSelectableFlags_Disabled : 0;
                auto pretty_variant_name =
                    ImState::InDefaultMode() && i < (int)renderer_variants_pretty.size()
                        ? renderer_variants_pretty[i].c_str()
                        : renderer_variants[i].c_str();
                if ((IMGUI_STATE(ImGui::Selectable,
                                 pretty_variant_name,
                                 i == last_active,
                                 item_flags) ||
                     find_new_supported) &&
    ...
    renderer_changed |=
        IMGUI_STATE1(ImGui::Checkbox, "force synchronous rendering", &synchronous_rendering);
    ...
    if (IMGUI_STATE_BEGIN_HEADER(ImGui::CollapsingHeader, "Filtering", &renderer->params.reprojection_mode, ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        renderer_changed |= IMGUI_STATE1(ImGui::Checkbox, "enable denoising", &enable_denoising);
#endif

        bool upscale_2x = renderer->options.render_upscale_factor == 2;
        if (IMGUI_STATE1(ImGui::Checkbox, "use 2x upscaling", &upscale_2x)) {
        ...
        renderer_changed |= IMGUI_STATE1(ImGui::Checkbox, "TAA", &renderer->options.enable_taa);
        ...
        other_changes |= IMGUI_STATE1(ImGui::Checkbox, "raster TAA pattern", &raster_taa_enabled);
        other_changes |= IMGUI_STATE1(ImGui::Checkbox, "unjittered raster pattern", &force_unjittered_raster);
...
int BasicApplicationState::add_variants(RenderBackend *renderer)
{
    ...
    for (; i < (int)variants.size(); ++i) {
        ...
        renderer_variants_pretty.push_back(i < (int)pretty_variants.size() ? pretty_variants[i]
                                                                           : variants[i]);
    }
    return i;
}

void BasicApplicationState::begin_after_initialization(Shell::DefaultArgs const &config_args,
...
bool BasicApplicationState::save_framebuffer_png(const char *prefix, RenderBackend *renderer)
{
    ...
    const bool available =
        bufferSize == renderer->readback_framebuffer(bufferSize, readback_buffer_byte.data());
    readbackScope.end();

    BasicProfilingScope saveScope;
    saveScope.begin();
    const bool written = WriteImage::write_png(
        prefix, fbSize.x, fbSize.y, fbSize.z, readback_buffer_byte.data());
    saveScope.end();

    return available && written;
}

bool BasicApplicationState::save_framebuffer_pfm(const char *prefix, RenderBackend *renderer)
{
    ...
    readbackScope.begin();
    const size_t nRead =
        renderer->readback_framebuffer(bufferSize, readback_buffer_float.data());
    readbackScope.end();

    bool available = nRead == bufferSize;
    if (nRead == bufferSize / renderer->options.render_upscale_factor /
                     renderer->options.render_upscale_factor) {
    ...
    BasicProfilingScope saveScope;
    saveScope.begin();
    const bool written = WriteImage::write_pfm(
        prefix, fbSize.x, fbSize.y, fbSize.z, readback_buffer_float.data());
    saveScope.end();
                                                 ...
                                                 ExrCompression compression)
{
    ...
    BasicProfilingScope saveScope;
    saveScope.begin();
    const bool written = WriteImage::write_exr(
        prefix, fbSize.x, fbSize.y, fbSize.z, readback_buffer_float.data(), compression);
    saveScope.end();
                                         ...
                                         ExrCompression compression)
{
    ...
    readbackScope.begin();
    const bool available =
        renderer->readback_aov(aovIndex, bufferSize, readback_buffer_half.data());
    readbackScope.end();

    BasicProfilingScope saveScope;
    saveScope.begin();
    const bool written = WriteImage::write_exr(
        prefix, fbSize.x, fbSize.y, fbSize.z, readback_buffer_half.data(), compression);
    saveScope.end();
    return available && written;
}

void BasicApplicationState::handle_mode_actions(const Shell &shell, RenderBackend *renderer)
{
    if (validation_mode) {
        if ((frame_ready || target_spp <= 0) && !validation_img_prefix.empty()) {
            std::ostringstream os;
            os << validation_img_prefix << "_" << std::setw(4) << std::setfill('0')
               << accumulated_spp;
        ...
        if (!profiling_img_prefix.empty()
            // In profiling mode, limit writing to once per second (at the end of the
            // keyframe).
            && (current_time + profiling_delta_time) >= std::ceil(current_time)) {
            std::ostringstream os;
            os << profiling_img_prefix << "_" << std::setw(4) << std::setfill('0')
        ...
        if (frame_ready) {
            std::ostringstream os;
            os << data_capture.img_prefix << "_" << std::setw(4) << std::setfill('0')
               << ImState::CurrentKeyframe() + 1;
            ...
            if (data_capture.albedo_roughness) {
                save_aov_exr((pf + "_albedo_roughness").c_str(),
                             renderer,
                             RenderGraphic::AOVAlbedoRoughnessIndex,
                             EXR_COMPRESSION_NONE);
            }
            if (data_capture.normal_depth) {
                save_aov_exr((pf + "_normal_depth").c_str(),
                             renderer,
                             RenderGraphic::AOVNormalDepthIndex,
                             EXR_COMPRESSION_NONE);
            }
            if (data_capture.motion) {
                save_aov_exr((pf + "_motion_jitter").c_str(),
                             renderer,
                             RenderGraphic::AOVMotionJitterIndex,
                             EXR_COMPRESSION_NONE);
            }
