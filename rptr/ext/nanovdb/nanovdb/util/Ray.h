...
public:
    ...
    /// @brief Return a new ray in index space, assuming the existing
    ///        ray is represented in the world space of the specified grid. (double version)
    template<typename GridType>
    __hostdev__ Ray worldToIndex(const GridType& grid) const
    {
        const Vec3T eye = grid.worldToIndex(mEye);
        const Vec3T dir = grid.worldToIndexDir(mDir);
        const RealT length = dir.length(), invLength = RealT(1) / length;
        RealT       t1 = mTimeSpan.t1;
        if (mTimeSpan.t1 < Maximum<RealT>::value()) {
            t1 *= length;
