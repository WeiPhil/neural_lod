...
struct RenderParams {
    ...
    int ray_query_method GLCPP_DEFAULT(= 0); // 0 = Throughput Optimisation (Ray Queries),  1 = Visibility Optimisation (Ray Queries)
    int _pad4;
    int _pad5;
};
...
struct RenderRayQueryResult {
    GLM(vec4) result; // color for voxelization or throughput for bssrdf voxelization
    GLM(vec3) normal;
    int second_order_sample;
    GLM(vec3) estimates_or_inference;
    int secondary_occ_hit;
    GLM(vec3) wi; // outgoing direction (incident light direction) for a voxel
    int occlusion_hit_or_pdf;
};
