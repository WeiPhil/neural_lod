    ...
    : device(backend->device), backend(backend), tracked_frames(0)
{
    ...
    for (uint32_t frame_idx = 0; frame_idx < stabilization_frames; ++frame_idx)
        memset(profiling_timings_ms[frame_idx],
               0,
               sizeof(float) * (uint32_t)ProfilingMarker::Count);
}
...
// processing pipeline
void ProcessProfilingToolsVulkan::register_custom_descriptors(
    vkrt::BindingLayoutCollector collector, vkrt::RenderPipelineOptions const &options) const
{
}
...
void ProcessProfilingToolsVulkan::process(CommandStream *cmd_stream_, int variant_idx)
{
    ...
    for (uint32_t marker_idx = 0; marker_idx < (uint32_t)ProfilingMarker::Count;
         ++marker_idx) {
    ...
    uint32_t available_stab_frames =
        (uint32_t)std::min(tracked_frames, size_t(stabilization_frames));
    uint32_t stab_window_begin =
        (stab_frame + stabilization_frames + 1 - available_stab_frames) % stabilization_frames;
    for (uint32_t marker_idx = 0; marker_idx < (uint32_t)ProfilingMarker::Count;
         ++marker_idx) {
        size_t avg_begin = 0;
        size_t avg_end = 0;
...
bool ProcessProfilingToolsVulkan::ui_and_state(bool &renderer_changed)
{
    ...
    IMGUI_STATE1(ImGui::Checkbox, "Enable Timeline", &params.enableTimeline);
    IMGUI_STATE1(ImGui::Checkbox, "Pause Capture", &params.pauseCapture);
    IMGUI_STATE1(ImGui::Checkbox, "Advanced Metrics", &params.advancedMetrics);
    ...
    if (ImState::InDefaultMode() && valid_frame && params.enableTimeline) {
        IMGUI_STATE1(ImGui::Checkbox, "Detailed View", &params.detailedView);
        ...
        for (uint32_t marker_idx = 0; marker_idx < (uint32_t)ProfilingMarker::Count;
            ...
            if (params.advancedMetrics)
                ImGui::Text("%s %6.3f ms (min: %6.3f ms, max: %6.3f ms, stddev: %6.3fms)",
                            get_profiling_marker_name((ProfilingMarker)marker_idx),
                            timings_ms,
                            min_timing_ms,
                            max_timing_ms,
                            stddev_timing_ms);
            else
                ImGui::Text("%s %6.3f ms",
                            get_profiling_marker_name((ProfilingMarker)marker_idx),
                            timings_ms);
            ...
            if (marker_start != marker_end) {
                ...
                draw_list->AddRectFilled(
                    ImVec2(p.x + start * frame_pixel_size, p.y),
                    ImVec2(p.x + frame_pixel_size * (start + duration), p.y + square_height),
                    col32,
                    10.0f);
            }
            // Spacing
        ...
        IMGUI_STATE1(ImGui::Checkbox, "Graph View", &params.graphView);
        if (params.graphView) {
            ...
            if (IMGUI_STATE_BEGIN_ATOMIC_COMBO(
                    ImGui::BeginCombo, "marker", marker_names, marker_names[last_active])) {
...
bool ProcessProfilingToolsVulkan::profiling_csv_declare_column_names(
