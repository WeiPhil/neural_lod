    ...
    , profiling_data(device)
{
    ...
    local_param_buf = vkrt::Buffer::host(*device,
                                        sizeof(glsl::LocalParams),
                                        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                        VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                        swap_buffer_count);
    global_param_buf = vkrt::Buffer::host(*device,
...
void RenderVulkan::internal_release_resources()
{
    ...
    vkDestroyDescriptorPool(device->logical_device(), debug_texture_desc_pool, nullptr);
    ...
    vkDestroyDescriptorSetLayout(device->logical_device(), debug_texture_desc_layout, nullptr);

    vkDestroySampler(device->logical_device(), sampler, nullptr);
    vkDestroySampler(device->logical_device(), screen_sampler, nullptr);
    vkDestroySampler(device->logical_device(), debug_texture_sampler, nullptr);
...
void RenderVulkan::enable_ray_queries(int max_queries, const int max_queries_per_pixel) {
    ...
    ray_result_buffer = vkrt::Buffer::device(memory_arena,
        max_query_budget * sizeof(RenderRayQueryResult),
        VK_BUFFER_USAGE_TRANSFER_SRC_BIT
        | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT
        | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT);
}

void RenderVulkan::create_debug_texture(){

    if (!debug_texture_desc_layout){
        debug_texture_desc_layout =
    ...
    if (!debug_texture_sampler)
    {
        ...
        CHECK_VULKAN(
            vkCreateSampler(device->logical_device(), &sampler_info, nullptr, &debug_texture_sampler));
    }
    ...
    auto sync_commands = device.sync_command_stream();
    {
        ...
        img_mem_barrier.image = debug_texture_buffer->image_handle();
        img_mem_barrier.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        img_mem_barrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        img_mem_barrier.srcAccessMask = 0;
        img_mem_barrier.subresourceRange.levelCount = 1;

        vkCmdPipelineBarrier(sync_commands->current_buffer,
                            VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                            VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                            0,
                            0, nullptr,
    ...
    CHECK_VULKAN(vkCreateDescriptorPool(
        device->logical_device(), &pool_create_info, nullptr, &debug_texture_desc_pool));

    // Create Descriptor Set:
    {
        VkDescriptorSetAllocateInfo alloc_info = {};
        alloc_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        alloc_info.descriptorPool = debug_texture_desc_pool;

        uint32_t texture_set_size = uint32_t(1);
        ...
        alloc_info.pSetLayouts = &debug_texture_desc_layout;
        alloc_info.pNext = &texture_set_size_info;
        CHECK_VULKAN(
            vkAllocateDescriptorSets(device->logical_device(), &alloc_info, &debug_texture_desc_set));
        alloc_info.pNext = nullptr;
    }

    vkrt::DescriptorSetUpdater updater;
    updater.write_combined_sampler_array(debug_texture_desc_set, 0, std::vector<vkrt::Texture2D>{debug_texture_buffer}, std::vector<VkSampler>{debug_texture_sampler});
...
bool RenderVulkan::render_ray_queries(int num_queries, const RenderParams &params, int variant_idx, CommandStream* cmd_stream_) {
    if (!cmd_stream_){
        if (variant_idx >= 0 && variant_idx < (int) GPU_RAYTRACER_NAMES.size())
            build_raytracing_pipeline(variant_idx, this->active_options, false);
...
void RenderVulkan::upload_light_data()
{
    ...
    if (light_data_buf == nullptr || light_data_buf.size() < lightDataSize)
        light_data_buf = vkrt::Buffer::device(reuse(static_memory_arena, light_data_buf), lightDataSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,0, swap_buffer_count);
