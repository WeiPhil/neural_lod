...
void Shell::readwrite_window_state()
{
    ...
    IMGUI_STATE1(ImGui::Checkbox, "window maximized", &win_maximized);
}
...
int main(int argc, const char **argv)
{
    ...
    const std::vector<std::string> default_ini_search_path = {"", "configs/"};
    ...
    println(CLL::INFORMATION,
            "Binary and resource paths: \"%s\", \"%s\"",
            get_executable_path(),
            shell.cmdline_args.resource_dir.c_str());

    ImState::RegisterApplicationSettings(*ImGui::GetCurrentContext());
    // skip default persistent state on validation
    if (shell.cmdline_args.validation_mode || shell.cmdline_args.profiling_mode ||
    ...
    auto load_ini_settings = [&]() {
        ...
        for (const auto &f : shell.cmdline_args.scene_files) {
            ...
            if (default_scene_ini_path.empty()) {
                println(CLL::INFORMATION,
                        "Cannot find default scene ini %s",
                        default_scene_ini.c_str());
            } else {
                println(CLL::INFORMATION,
                        "Loading default scene ini %s",
    ...
    else if (shell.cmdline_args.validation_mode || shell.cmdline_args.profiling_mode ||
             shell.cmdline_args.data_capture_mode) {
    ...
    shell.window = glfwCreateWindow(
        shell.win_width, shell.win_height, "Neural Prefiltering", nullptr, nullptr);
    if (!shell.window) {
        char const *error_msg = "unknown";
        glfwGetError(&error_msg);
...
std::unique_ptr<RenderBackend> Shell::create_standard_renderer(std::string const &name,
                                                               Display *display)
{
