...
parser.add_argument('--envmap_file', default="kloppenheim_06_puresky_2k.exr", help="envmap name located at ../../neural_lod/hdri/")
parser.add_argument('--envmap_rotation', type=float, default=0.0, help='optional envmap rotation (default: 0.0)')
...
envmap_filename = "../../neural_lod/hdri/{}".format(args.envmap_file)

envmap_rotation = args.envmap_rotation
    ...
    for i in range(0,len(data)):
        ...
        if(data[i].startswith("apply visibility russian roulette=")):
            if(not args.no_auto_vis_rr and not args.no_vis_rr):
                data[i] = "apply visibility russian roulette= 1\n" if res >= 64 else "apply visibility russian roulette= 0\n"
            else:
        ...
        if(data[i].startswith("[.][Wavefront Neural Throughput Visibility Lod]")):
            if(data[i+1].startswith("use envmap= 0")):
                data[i] = "[.][Wavefront Neural Throughput Visibility Lod]\nuse envmap= 1\n"
                data[i+1] = "envmap filename={}\nenvmap rotation= {}\n".format(envmap_filename,envmap_rotation)
            elif(data[i+1].startswith("use envmap= 1") and data[i+2].startswith("envmap filename=")):
                data[i+2] = "envmap filename={}\nenvmap rotation= {}\n".format(envmap_filename,envmap_rotation)
        if(data[i].startswith("[.][Wavefront Neural Ref]")):
            if(data[i+1].startswith("use envmap= 0")):
                data[i] = "[.][Wavefront Neural Ref]\nuse envmap= 1\n"
                data[i+1] = "envmap filename={}\nenvmap rotation= {}\n".format(envmap_filename,envmap_rotation)
            elif(data[i+1].startswith("use envmap= 1") and data[i+2].startswith("envmap filename=")):
                data[i+2] = "envmap filename={}\nenvmap rotation= {}\n".format(envmap_filename,envmap_rotation)
