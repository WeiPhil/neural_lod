...
void printHelp(){
	...
	cout << " -pad : Adds a small epsilon padding to the mesh's AABox (" << to_string(epsilonPadding) << " times it's extent)" << endl << endl;
	printExample();
	cout << endl;
}
...
void parseProgramParameters(int argc, char* argv[]){
	...
	for (int i = 1; i < argc; i++) {
		...
		else if (string(argv[i])=="-pad"){
			useEpsilonPadding = true;
		}
	}
	if (!filegiven) {
...
int main(int argc, char* argv[]) {
	...
	if(useEpsilonPadding){
		fprintf(stdout, "/!\\ Apllying a small padding to the AABox : %f of it's extent \n", epsilonPadding);
		filename += "_eps_padded";
	}
	// Initialize our own AABox, pad it so it's a cube
	AABox<glm::vec3> bbox_mesh_cubed = createMeshBBCube<glm::vec3>(AABox<glm::vec3>(trimesh_to_glm(themesh->bbox.min), trimesh_to_glm(themesh->bbox.max)), useEpsilonPadding ? epsilonPadding : 0.f);
